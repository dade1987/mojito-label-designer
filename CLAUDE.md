...
`src/components/LabelDesigner.vue` is the orchestrating component (state, toolbar, save/load flows, API calls); `src/components/LabelCanvas.vue` renders the drag/select/position canvas. All side panels live in a **single left column** as collapsible `<details class="section">` blocks (Stampa manuale, Layout, Proprietà, Elementi, Data source, Etichetta, Anteprima, API & ZPL); which ones are open is persisted by `src/utils/panelSections.js`. Interactive controls are sized to a 44px minimum touch target (WCAG 2.5.5) — the stations are touched with gloves on. Canvas geometry helpers (hit-testing, selection rectangles, placement) are split into `src/utils/canvasDisplay.js`, `canvasSelection.js`, and `elementPlacement.js` for the same testability reason.
